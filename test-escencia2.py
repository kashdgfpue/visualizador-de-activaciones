...
class EsenciaApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Extractor de Esencia Multimodal - Versión Mejorada")
    ...
    def _actualizar_controles(self):
        ...
        # Solo habilitar procesar si hay datos disponibles
        procesar_disabled = True
        if fuente == 'archivo' and hasattr(self, 'archivo_path') and self.archivo_path:
            procesar_disabled = False
        elif fuente == 'camara' and self.captured_frame is not None:
            procesar_disabled = False
            
        self.procesar_btn.config(state='normal' if not procesar_disabled else 'disabled')
    ...
    def capturar_imagen(self):
        if self.cap is not None and self.capturing:
            ret, frame = self.cap.read()
            if ret:
    ...
    def extraer_esencia(self):
        ...
        try:
            ...
            if fuente == 'archivo':
                if not hasattr(self, 'archivo_path') or not self.archivo_path:
                ...
                descripcion = f"archivo {os.path.basename(self.archivo_path)}"
            
            elif fuente == 'camara':
                if tipo == 'imagen' and self.captured_frame is not None:
                    input_data = self.captured_frame
                    descripcion = "imagen de cámara"
                else:
                    raise ValueError("No hay imagen capturada de la cámara")
            
            if input_data is None:
                raise ValueError("No hay datos para procesar")
            
            self.procesar_btn.config(state='disabled')
            self.result_text.insert(tk.END, f"\nIniciando procesamiento de {tipo} ({descripcion})...\n")
